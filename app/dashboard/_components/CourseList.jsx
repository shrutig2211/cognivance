...
function CourseList() {
  ...
  const GetCourseList = async () => {
    // get a safe email value (try primary, then first emailAddresses entry)
    const email =
      user?.primaryEmailAddress?.emailAddress ||
      user?.emailAddresses?.[0]?.emailAddress;

    if (!email) {
      console.warn("GetCourseList: no user email available yet.");
      return;
    }
    ...
    try {
      ...
      setCourseList(list);
      if (typeof setTotalCourse === "function") {
